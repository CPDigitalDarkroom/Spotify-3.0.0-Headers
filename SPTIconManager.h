//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@interface SPTIconManager : NSObject
{
}

+ (id)spoticonFont;
+ (id)keyFromConfiguration:(id)arg1;
+ (id)sharedCache;
+ (id)iconWithConfiguration:(id)arg1;
+ (id)stringForIcon:(unsigned int)arg1 size:(unsigned int)arg2;
+ (unsigned short)characterForIcon:(unsigned int)arg1 size:(unsigned int)arg2;
+ (id)dictionaryOfCharacterMappings;
+ (id)valueWithCharacterSetFromCharacters:(unsigned int)arg1;
+ (struct SPTIconCharacterSet)patchSet:(struct SPTIconCharacterSet)arg1;

@end

