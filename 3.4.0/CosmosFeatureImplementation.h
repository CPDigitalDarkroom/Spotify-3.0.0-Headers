//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "CosmosFeature.h"
#import "SPTFeature.h"

@class NSString, SPTAllocationContext, SPTRouterFactory;

@interface CosmosFeatureImplementation : NSObject <SPTFeature, CosmosFeature>
{
    SPTRouterFactory *_routerFactory;
    id <BaseFeature> _baseFeature;
}

+ (id)featureDependencies;
+ (id)featureIdentifier;
@property(retain, nonatomic) id <BaseFeature> baseFeature; // @synthesize baseFeature=_baseFeature;
@property(retain, nonatomic) SPTRouterFactory *routerFactory; // @synthesize routerFactory=_routerFactory;
- (void).cxx_destruct;
- (id)provideRouterFactory;
- (id)provideRouterWithParameters:(id)arg1;
- (id)provideResolverWithParameters:(id)arg1;
- (id)provideResolverWithViewName:(id)arg1 viewSession:(id)arg2 viewVersion:(id)arg3;
- (void)unload;
- (void)configureWithFeatures:(id)arg1;

// Remaining properties
@property(retain, nonatomic) SPTAllocationContext *allocationContext;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

