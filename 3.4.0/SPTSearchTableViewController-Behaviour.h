//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "SPTSearchTableViewController.h"

@interface SPTSearchTableViewController (Behaviour)
@property(readonly, nonatomic) _Bool shouldAdjustViewsForKeyboard;
@property(readonly, nonatomic) _Bool shouldDismissKeyboardWhenTappingHeaderOrFooter;
@property(readonly, nonatomic) _Bool shouldDismissKeyboardWhenSelectingSearchHistoryRow;
@property(readonly, nonatomic) _Bool shouldDismissKeyboardWhenSelectingSearchResultRow;
@property(readonly, nonatomic) _Bool shouldDismissKeyboardWhenScrollingContent;
@property(readonly, nonatomic) _Bool shouldDismissKeyboardWhenSearching;
@end

